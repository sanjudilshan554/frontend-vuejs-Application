<template>
  <h1 class="h1">Revision Accepted</h1>
  <div class="card cs">
    <div class="card-body">
          ...
          <tr v-for="values in requesterdetail" :key="values.id">
            <td>
              <div class="d-flex align-items-center">
                ...
                />
                <div class="ms-3">
                  <p class="fw-bold mb-1">{{ values.lectureName }}</p>
                  <p class="text-muted mb-0">{{ values.lecturemail }}</p>
                </div>
              </div>
            </td>
            <td>
              <p class="fw-normal mb-1">{{ values.Revision_name }}</p>
            </td>
            <td>{{ values.Subject }}</td>
            <td>{{ values.request_time }}</td>
            <td>{{ values.created_at }}</td>
            <td>{{ values.request_place }}</td>
            <td>{{ values.mobNumber }}</td>
            <td>{{ values.Revision_namemembers }}</td>
            <td>
              <button
                type="button"
...
export default {
  ...
  mounted() {
    ...
    axios
      .post("http://127.0.0.1:8000/api/RevisionAcceptviewer", this.form)
      .then((response) => {
        if (response.status == "200") {
          this.requesterdetail = response.data.data;
          this.lecturedetail = response.data.data2;
        }
      })
      .catch((error) => {
        console.log(error);
